...
std::tuple<double, std::string> get_freq_hardware_from_filename(const std::string& filename)
{
	...
	if (sp != std::string::npos)
	{
		...
		{
			std::cout << "The filename does not have hardware information (rtlsdr/hackrf/bladerf/usrp)!" << "\n";
		}
	}
...
cvec get_signal_from_bin(const std::string& filename, size_t num_sample_read, const std::string& dev)
{
	...
	if (!file.is_open())
	{
		std::cout << "get_signal_from_bin: Can not open file!" << "\n";
		return s;
	}
	...
	// {
	// 	s.clear();
	// 	std::cout << "get_signal_from_bin: Not enough samples in the file!" << "\n";
	// }
...
cvec filter_wo_tail(cvec s, vec& coef, double convert_ratio)
{
	...
	if (s.size() == 1)
	{
		std::cerr << "filter_wo_tail: input signal must be column vector" << "\n";
		return r;
	}
	const size_t len_coef = coef.size();
	if (len_coef % 2 == 0)
	{
		std::cerr << "filter_wo_tail: length of coef must be odd!" << "\n";
		return r;
	}

	if (convert_ratio <= 1)
	{
		const int downsampling_ratio = static_cast<int>(std::round(1 / convert_ratio));
		const cvec tmp(s);
...
void sampling_ppm_f_search_set_by_pss(
	// input
	cvec s, const vec& fo_search_set, cmat& td_pss, cmat pss_fo_set, int sampling_carrier_twist,
	complex<double> pss_peak_max_reserve,
...
{
	const size_t len_pss = pss_fo_set.rows();
	size_t num_fo_pss = pss_fo_set.cols();

	const size_t len = s.size();
	const size_t len_short = len - (len_pss - 1);
	...
	if (sampling_carrier_twist == 1)
	{
		...
		for (size_t foi = 0; foi < n_f; foi++)
		{
			...
			{
				const int col_idx = t * fo_search_set.size() + fo_idx_set[foi];
...
void xcorr_pss(
	// input
	const cvec& capbuf, const vec& f_search_set, int DS_COMB_ARM, double& fc, int sampling_carrier_twist,
	double k_factor, vector<mat>& xc,
	// output
	mat& xc_incoherent_collapsed_pow, vector<ivec>& xc_incoherent_collapsed_frq, double& n_comb_xc, vec& sp_incoherent,
	vec& sp)
{
	const int n_cap = capbuf.size();
	const size_t n_f = f_search_set.size();
	...
	for (size_t foi = 0; foi < n_f; foi++)
	{
		...
		for (size_t t = 0; t < 3; t++)
		{
			...
			for (size_t m = 0; m < n_comb_xc; m++)
			{
				...
				const double actual_time_offset = m * 0.005 * k_factor;
				const double actual_start_index = itpp::round_i(actual_time_offset * FS_LTE / 16);
	...
	const int n_comb_sp = floor(length(sp) / 9600);
	...
	list<Cell>& cells)
{
	...
	for (;;)
	{
		...
		for (int t = 0; t <= 2; ++t)
		{
			if (t != peak_n_id_2)
			{
				const double tmp = peak_pow * pss_xc_matrix(peak_n_id_2, t);
				for (size_t i = 0; i < cancel.size(); i++)
				{
		...
		const double tmp = peak_pow * myudb10(-12);
                  ...
                  complex<double> par_th, int num_peak_th)
{
	...
	sampling_ppm_f_search_set_by_pss(capbuf_pbch, f_search_set, td_pss, pss_fo_set, sampling_carrier_twist,
	                                 pss_peak_max_reserve, num_pss_period_try, combined_pss_peak_range, par_th,
	                                 num_peak_th, period_ppm, dynamic_f_search_set, xc);
	...
	vec sp_vec(9600);
	xcorr_pss(capbuf_pbch, f_search_set, DS_COMB_ARM, fc, sampling_carrier_twist, 0.0, xc, xc_incoherent_collapsed_pow,
	          xc_incoherent_collapsed_frq, n_comb_xc, sp_incoherent, sp_vec);
	...
	// remove /2 to avoid many false alarm

	list<Cell> peaks;
	peak_search(xc_incoherent_collapsed_pow, xc_incoherent_collapsed_frq, Z_th1, dynamic_f_search_set, fc,
	            sampling_carrier_twist, peaks);
	...
	std::stringstream ss;
	ss << "-------------------------------Cells information summary-------------" << peaks.size() << "\n";
	for (const auto& peak : peaks)
	{
		ss << "Cell " << i++ << " information:--------------------------------------------------------" << "\n";
		ss << "            Cell mode: " << (peak.duplex_mode == 1 ? "TDD" : "FDD") << "\n";
		...
		ss << "       RX power level: " << 10 * log10(peak.pss_pow) << "\n";
...
void showDuration()
{
    ...
    // 输出执行时间（微秒）
    std::cout << "执行时间: " << duration.count() / 1000 << " 微秒" << std::endl;
}
...
__declspec(dllexport) const char* search(char* s)
{
	...
	const int sampling_carrier_twist = 1;
	const double num_radioframe = 8;
	double num_second = num_radioframe * 10e-3;
	double raw_sampling_rate = 19.2e6;
	int nRB = 100;
	const double sampling_rate = 30.72e6;
	const double sampling_rate_pbch = sampling_rate / 16; // LTE spec. 30.72MHz/16.
	double bandwidth = 20e6;

	const int pss_peak_max_reserve = 2;
	const int num_pss_period_try = 1;
	const int combined_pss_peak_range = -1;
	const double par_th = 8.5;
	const double num_peak_th = 1 / 2;
	...
	{
		std::cout << filename << " does not include valid frequency or hardware info!" << "\n";
	...
	// std::cout << "fc " << fc << "; IQ from " << sdr_board << " " << filename << "\n";
	const vec coef_pbch = fir1(254, (0.18e6 * 6 + 150e3) / raw_sampling_rate);
	const vec coef_8x_up = fir1(254, 20e6 / (raw_sampling_rate * 8));

	const vec f_search_set = "-140e3:5e3:135e3";
	...
	else
	{
		...
		const ivec indices = itpp_ext::matlab_range(0, 5, r_20M.size());
		// ivec indices = to_ivec(linspace(0, r_20M.size(), r_20M.size() / 5));
		r_20M = r_20M(indices);
	}
	std::cout << "CellSearch before\n";
	showDuration();
	...
	const string res = CellSearch(r_pbch, r_20M, f_search_set, fc, sampling_carrier_twist, pss_peak_max_reserve,
	                              num_pss_period_try, combined_pss_peak_range, par_th, num_peak_th);
	std::cout << "CellSearch done\n";
	showDuration();
