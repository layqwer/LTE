				...
				complex<double> par_th, int num_peak_th)
{
	...
	{
		// todo:
		//  value.extra_info.num_peaks_raw = peaks.size()/2;
	}
	...
	for (auto &peak : peaks)
	{
		...
		if (peak.n_id_1 != 0)
		{
			...
			peak = decode_mib(peak, tfg, rs_dl);
			if (peak.n_rb_dl != 0)
			{
				continue;
			}
	...
	std::cout << "-------------------------------Cells information summary-------------" << peaks.size() << std::endl;
	for (const auto &peak : peaks)
	{
		std::cout << "Cell " << i++ << " information:--------------------------------------------------------" << std::endl;
		std::cout << "            Cell mode: " << (peak.duplex_mode == 1 ? "TDD" : "FDD") << std::endl;
		std::cout << "              Cell ID: " << peak.n_id_cell() << endl;
		std::cout << "   Num. eNB Ant ports: " << peak.n_ports << endl;
		std::cout << "    Carrier frequency: " << fc / 1e6 << "MHz" << endl;
		std::cout << "Residual freq. offset: " << peak.freq_superfine / 1e3 << "kHz" << endl;
		std::cout << "       RX power level: " << 10 * log10(peak.pss_pow) << std::endl;
		std::cout << "              CP type: " << peak.cp_type << endl;
		std::cout << "              Num. RB: " << peak.n_rb_dl << endl;
		std::cout << "       PHICH duration: " << peak.phich_duration << endl;
		std::cout << "  PHICH resource type: " << peak.phich_resource << endl;
	}
}
