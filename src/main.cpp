...
cvec filter_wo_tail(cvec s, vec &coef, double convert_ratio)
{
	...
	if (convert_ratio <= 1)
	{
		...
		r = r(to_ivec(linspace(0, r.size(), r.size() / downsampling_ratio)));
	}
	else
	{
	...
	size_t &ppm, vec &f_set, vector<mat> &xc)
{
	...
	if (sampling_carrier_twist == 1)
	{
		...
		f_set = fo_search_set;
		vec fo_idx_set = itpp::linspace(0, f_set.size() - 1, f_set.size());
				...
				complex<double> par_th, int num_peak_th)
{
	...
	cout << capbuf_pbch[0] << capbuf_pbch[1] << capbuf_pbch[2] << capbuf_pbch[3] << capbuf_pbch[4] << endl;
	...
	cout << xc_incoherent_collapsed_frq[0] << endl;
	cout << xc_incoherent_collapsed_frq[1] << endl;
	cout << xc_incoherent_collapsed_frq[2] << endl;

	std::cout << "-------------------------------Cells information summary-------------" << cells.size() << std::endl;
...
int main()
{
	...
	else
	{
		...
		vec indices = linspace(0, r_20M.size(), r_20M.size() / 5);
