...
cvec testCsv()
{
    std::ifstream file("./output_data.csv", std::ios::binary);
    if (!file.is_open())
    {
    ...
    cvec s(numRows);
    regex complexRegex("([-+]?[0-9]*\\.?[0-9]+(?:[eE][-+]?[0-9]+)?)([-+]([0-9]*\\.?[0-9]+(?:[eE][-+]?[0-9]+)?))?i");

    int index = 0;
    while (getline(file, line))
    {
        smatch match;
        if (regex_search(line, match, complexRegex))
        {
            // 获取匹配的复数字符串
            string realStr = match[1].str();
            string imagStr = match[2].str();

            // 将实部和虚部字符串转换为double，并构建复数
            double realPart = stod(realStr);
            double imagPart = stod(imagStr);

            // 将复数添加到数据向量中
            s[index++] = complex<double>(realPart, imagPart);
        }
        else
        {
            cout << line << endl;
...
mat fft_corr(cvec s, cmat &td_pss, const vec &fo_search_set)
{
    ...
    s = fft(s);
    // s = testCsv();
    // cout << "s " << s(0) << " " << s(992) << " " << s(4003) << " " << s(9994) << " " << s(30000) << " " << s(150000) << " " << s(153599) << endl;
...
void extract_tfg(Cell &peak, cvec capbuf, double fc, int sampling_carrier_twist, int nRB, cmat &tfg, vec &tfg_timestamp)
{
    ...
    cout << "tfg_t " << tfg_t << endl;
    tfg = concat_horizontal(tfg_t.get_cols(-1 - ((nSC / 2) - 1), -1), tfg_t.get_cols(1, (nSC / 2)));
    cout << "tfg " << tfg << endl;
